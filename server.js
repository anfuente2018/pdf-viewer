...
const MONGO_URL =
  'mongodb://anfuente:anfuente123456@ds155213.mlab.com:55213/anfuente'
...
app.post('/uploadpdf', upload_pdf.single('pdf'), (req, res, next) => {
  try {
    ...
    db.insert(datos, (err, doc) => {
      ...
      console.log()
      console.log(
        `PDF Insertado|Nro:${doc.nro}|Cod:${doc.cod}|Tipo:${doc.type}|Fecha:${doc.fulldate}`
      )
      console.log(doc)
    })
...
app.get('/onedrive/files/:name', async (req, res) => {
  let pag = !req.query.pag ? 1 : req.query.pag
  // let limit = !req.query.limit ? 20 : req.query.limit
...
app.get('/temp/data', async (req, res) => {
  let pdf = await tempdata.getTemp()
  res.json({
    res: true,
    pdf
  })
